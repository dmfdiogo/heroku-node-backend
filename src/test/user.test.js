...
describe('Users', () => {
        ...
        it('when no data is passed', async () => {
            try {
                ...
                expect(res.body)
                    ...
                    .to.equal('Please fill all the fields')
            } catch (error) {
                throw error
            }
        })
    ...
    describe('Login User', () => {
        it('When email is not registerd', async () => {
            ...
            expect(res.body)
                .to.have.property('error')
                .to.equal('User with this email does not exist')
        })
        ...
        it('When email is registered', async () => {
            ...
            expect(res.body)
                .to.have.property('message')
                .to.equal('User loggedIn successfully')

            loggedInUserToken = res.header['set-cookie'][0]
                .split(';')[0]
                .split('=')[1]
        })
    })
    ...
    describe('User Profile', () => {
        it('Get Loggedin User Profile', async () => {
            const res = await chai
                .request(BASE_URL)
                .get('/profile')
                .set('cookie', `token=${loggedInUserToken}`)
